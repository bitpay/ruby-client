...
describe BitPay::SDK::Client do
  ...
  describe "#pair_client" do
    ...
    it 'throws a BitPayError with the error message if the token setting fails' do
      ...
      expect { bitpay_client.pair_client({pairingCode: :claim_code}) }.to raise_error(BitPay::BitPayError, '500: Unable to create token')
    end 
    ...
    it 'gracefully handles 4xx errors' do
      ...
      expect { bitpay_client.pair_client({pairingCode: :claim_code}) }.to raise_error(BitPay::BitPayError, '403: this is a 403 error')
    end
